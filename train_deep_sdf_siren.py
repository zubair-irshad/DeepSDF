...
import logging
import wandb
wandb.init(project="DeepSDF_all_without_scale_siren", entity="mirshad")
...
def main_function(experiment_directory, continue_from, batch_split):
    ...
    arch = __import__("networks." + specs["NetworkArch"], fromlist=["SirenNet"])
    ...
    decoder = arch.SirenNet(
        dim_in=67,  # input dimension, ex. 2d coor
        dim_hidden=512,  # hidden dimension
        dim_out=1,  # output dimension, ex. rgb value
        num_layers=6,  # number of layers
        final_activation=nn.Identity(),  # activation of final layer (nn.Identity() for direct output)
        w0_initial=3000.  # different signals may require different omega_0 in the first layer - this is a hyperparameter
    ).to(device)
