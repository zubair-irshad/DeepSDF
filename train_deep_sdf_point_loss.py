...
import logging
import wandb
wandb.init(project="all_ws_no_reg_pointloss", entity="mirshad")
...
def main_function(experiment_directory, continue_from, batch_split):
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for sdf_data, indices, points in sdf_loader:
            ...
            indices_sdf = torch.chunk(
                indices.unsqueeze(-1).repeat(1, num_samp_per_scene).view(-1),
                batch_split,
            )
            indices_points = torch.chunk(
            ...
            for i in range(batch_split):
                ...
                batch_vecs_points = lat_vecs(indices_points[i].to(device))
                with torch.no_grad():
                    _, pred_points = estimator(in_pc, emb=batch_vecs_points)
                
                batch_vecs = lat_vecs(indices_sdf[i].to(device))
                ...
                loss_points, _ = chamfer_distance(pred_points, points[i].to(device = device, dtype=torch.float))
                ...
                total_loss = chunk_loss + loss_points
                chunk_loss.backward()
                batch_loss += total_loss.item()
                sdf_losses += chunk_loss.item()
                point_losses+=loss_points.item()
                # print("reg_loss, epoch", reg_loss, epoch)
            # logging.info("loss = {}".format(batch_loss))
            logging.debug("loss = {}".format(batch_loss))
            loss_log.append(batch_loss)
            loss_values.append(batch_loss)
            sdf_loss_values.append(sdf_losses)
            point_loss_values.append(point_losses)
            if grad_clip is not None:
        ...
        wandb.log({"sdf_loss": np.mean(sdf_loss_values)})
        wandb.log({"points_loss": np.mean(point_loss_values)})
