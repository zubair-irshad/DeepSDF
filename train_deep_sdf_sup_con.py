...
from deep_sdf.losses import SupConLoss
import logging
import wandb
...
wandb.init(project="all_ws_complete", entity="mirshad")
...
def main_function(experiment_directory, continue_from, batch_split):
    ...
    sdf_dataset = deep_sdf.data.SDFSamplesSupCon(
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for sdf_data, indices, labels in sdf_loader:
            ...
            for i in range(batch_split):
                ...
                loss_sup_cont = criterion(F.normalize(batch_vecs_points, dim=1), labels[i].to(device))/batch_vecs_points.shape[0]
                ...
                total_loss = chunk_loss+0.1*loss_sup_cont
                total_loss.backward()
                batch_loss += total_loss.item()
                sdf_losses += chunk_loss.item()
                sup_con_losses+=loss_sup_cont.item()
            ...
            point_loss_values.append(sup_con_losses)
            if grad_clip is not None:
        ...
        wandb.log({"sup_con_loss": np.mean(point_loss_values)})
